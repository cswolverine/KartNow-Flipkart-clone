    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.orderNow.setOnClickListener((v)->{
            ...
            for(Product pd: favoriteList){
                productidList.add(pd.getProductId());
            }
            Log.d("list",productidList.size()+"");
            Intent shippingIntent = new Intent(CartActivity.this, ShippingAddressActivity.class);
            shippingIntent.putIntegerArrayListExtra(PRODUCTID, productidList);
            shippingIntent.putExtra("totalPrice",Integer.parseInt(binding.totalPrice.getText().toString()));
            Log.d("product",productidList+"");
        ...
        if (isNetworkConnected(this)) {
            cartViewModel.getProductsInCart(LoginUtils.getInstance(this).getUserInfo().getId()).observe(this, cartApiResponse -> {
                if (cartApiResponse != null) {
                    ...
                    cartAdapter = new CartAdapter(getApplicationContext(),binding.totalPrice, favoriteList, product -> {
